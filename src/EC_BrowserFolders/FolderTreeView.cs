...
namespace BrowserFolders
{
    ...
    public class FolderTreeView1
    {
        ...
        public FolderTreeView1(string topmostPath, string userData)
        {
            if (!userData.StartsWith(topmostPath, StringComparison.OrdinalIgnoreCase)) throw new ArgumentException("default path has to be inside topmost path");
        ...
        public string UserData
        {
            ...
            set
            {
                if (value != null) _userData = Path.GetFullPath(value.TrimEnd('\\'));
                else _userData = null;
            }
        }
        public Action CurrentFolderChanged;
        ...
        public void DrawDirectoryTree1()
        {
            ExpandToCurrentFolder1();

            _treeScrollPosition = GUILayout.BeginScrollView(
                _treeScrollPosition, GUI.skin.box,
                GUILayout.ExpandHeight(true), GUILayout.ExpandWidth(true));
            {
                DisplayObjectTreeHelper(new DirectoryInfo(UserData), 0);
            }
            GUILayout.EndScrollView();
        }

        private void ExpandToCurrentFolder1()
        {
            var path = CurrentFolder;
            var defaultPath = UserData;
            _openedObjects.Add(defaultPath.ToLowerInvariant());
            while (!string.IsNullOrEmpty(path) && path.Length > defaultPath.Length)
            {
                _openedObjects.Add(path.ToLowerInvariant());
        ...
        private void DisplayObjectTreeHelper(DirectoryInfo dir, int indent)
        {
            ...
            if (indent == 0 && subDirs.Length == 0)
            {
                GUILayout.BeginVertical();
                {
                    GUILayout.Label(@"You can organize your files into folders and use this window to browse them.");
                    GUILayout.Space(5);
                    GUILayout.Label($@"Folders placed inside your {UserData.Substring(_topmostPath.Length)} folder will appear on this list.");
                }
                GUILayout.EndVertical();
                return;
            }
